                        ...
                        for m in range(len(liste_chemin_crit[n])):
                            if chemin_crit[i] == liste_chemin_crit[n][len(liste_chemin_crit[n])-1-added]:
                                ...
                                if chemin_crit[i] == liste_chemin_crit[n][len(liste_chemin_crit[n])-1-m-added]:
                                    if added == 0:
                                        print("WHAT", chemin_crit[i], liste_chemin_crit[n][len(liste_chemin_crit[n])-1-m-added])
                                        print("COPY", copy.deepcopy(liste_chemin_crit[n][:len(liste_chemin_crit[n])-added-m-1]), "APPENDING", chemin_crit[j])
                                        liste_chemin_crit.append(copy.deepcopy(liste_chemin_crit[n][:len(liste_chemin_crit[n])-added-m-1]))
