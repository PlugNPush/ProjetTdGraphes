...
def critique() :
    ...
    with open("L3-C4-trace{}.txt".format(Gv.num_file),"a") as Trace :
        ...
        for e in range(len(liste_chemin_crit)):
            if liste_chemin_crit[e] not in liste_finale_chemin_crit:
                print(chemin_crit[len(chemin_crit)-1])
                liste_finale_chemin_crit.append(liste_chemin_crit[e].append(chemin_crit[len(chemin_crit)-1]))
