...
def dates() :
    with open("L3-C4-trace{}.txt".format(Gv.num_file),"a") as Trace :
        ...
        Trace.write("Dates au plus tot  : " + str(["{:.2f}".format(v) for v in dates_tot]) + "\nDates au plus tard : " + str(["{:.2f}".format(v) for v in dates_tard]) + "\nMarge              : " + str(["{:.2f}".format(v) for v in marge]) + "\n")
        Trace.write("\n")
